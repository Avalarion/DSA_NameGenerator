...
jQuery('body').on('click', ".saved", function(){
	var tmp = localStorage.getItem('generatorNames').replace(jQuery(this).html(), '');
	tmp = tmp.replace(/,,/, ',');
...
jQuery(window).scroll(function() {
    if(jQuery(window).scrollTop() == jQuery(document).height() - jQuery(window).height() && !blockScroll) {
        attachNames(20);
    }
});
	...
	for(var i = 0; i < loadedNames.length; i++) {
		if(loadedNames[i] != 'null' && loadedNames[i] != '' ) {
			jQuery('#results').append(
				jQuery('<div class="nameEntry saved large-12 columns"></div>').html(loadedNames[i])
			);
		}
	}
}
